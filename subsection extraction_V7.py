...
pdf = "1-mvr-part-4-jul24_436.pdf"
doc = fitz.open(pdf)
    ...
    for i, block in enumerate(text_blocks):
        ...
        if (12.0, "Arial-Black") in level_font_dict_all:
            ...
            if (11.0, "Arial-BoldMT") in level_font_dict_all and not flag_2_red_done:
                rect_2 = rect
                image_2 = doc[page_num].get_pixmap(clip=fitz.Rect(rect_2))
            ...
            if (10.0, "Arial-BoldMT") in level_font_dict_all and not flag_3_red_done:
                rect_3 = rect
                image_3 = doc[page_num].get_pixmap(clip=fitz.Rect(rect_3))
        ...
        if block_flag:
            ...
            # print out the fixed red content                    
            image = doc[page_num].get_pixmap(clip=fitz.Rect(rect))
...
def insert_images_to_word(images, word_file_path):
    doc = Document()
    ...
    for image_bytes in images:
        ...
        # Calculate the new image size to fit within the Word page (8.5 x 11 inches)
        # Word document size is typically 6.5 x 9 inches for content area
        max_width = 6.5 * 72  # 6.5 inches in points (1 inch = 72 points)
        max_height = 9 * 72   # 9 inches in points
        
        # Get current image size
        img_width, img_height = img.size
        scale_ratio = 1   # scaling factor is the image is too large
        
        # Maintain aspect ratio while scaling the image to fit
        ...
        with BytesIO() as temp_image:
            img = img.resize((int(scale_ratio * img_width), int(scale_ratio * img_height)))
            img.save(temp_image, format='PNG')
            temp_image.seek(0)
            doc.add_picture(temp_image)
...
# Example usage
word_file_path = 'word output.docx'
insert_images_to_word(images, word_file_path)